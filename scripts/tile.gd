...
@export var _particles_main: PackedScene ;  var PARTICLES_main
@export var _particles_move: PackedScene ;  var PARTICLES_move
@export var _bump: PackedScene      ;  var BUMP

@onready var shader_revealed    = preload("res://assets/shaders/blur.gdshader")
...
#	allow_move_particles  = false
	...
	$"%Sprite2D".flip_v    = true if randf() > .5 else  false
	$"%Sprite2D".flip_h    = true if randf() > .5 else  false
	$"%Sprite2D".modulate  = Color(.4, .4, .4, 0)

	if theme_data[0] == 1:
		if theme_data[2] == false:
			$"%Sprite2D".texture = load(path + str(theme) + ".png")
		else:
			$"%Sprite2D".texture = load(path + str(theme) + "_OFF.png")
	else:
		var a = randi() % theme_data[0] + 1
		$"%Sprite2D".texture = load(path + str(theme) +"_"+ str(a) + ".png")
...
func _physics_process(delta: float) -> void:
	...
	velocity = velocity.lerp(Vector2.ZERO, damp *delta)
	...
	if z_offset < 0:
		...
		if v_velocity> 100:
			...
			tile_bounce_sound.pitch_scale  = randf_range(1, 3)
...
func io_anim(type := 0) -> void:
	if type == 0:
		...
		tween_io.tween_property(self, "rotation_degrees", def_rot,
			randf_range(.44, .82)).set_delay(.1)

		tween_io.tween_property(self, "scale", def_sca, .3)

		#tile spawn sound
		if Time.get_ticks_msec() >= level.sound_timeout:
			level.sound_timeout  = Time.get_ticks_msec() + 12
			tween_io.tween_callback(Callable($"../sounds/TileMain", "play")).set_delay(val)
		...
		tween_io.tween_callback(Callable(self, "tile_ready")).set_delay(.04)
...
func idle_anim() -> void:
	...
	if allow_idle_anim:
		...
		tween_idle.tween_property(
			self,"scale",
			sca *randf_range(.92, 1.04), time *.6)

		tween_idle.chain().tween_callback(Callable(self, "idle_anim"))
...
func tile_color_anim_accents(dist:float, smoothness:float, time:float, init:=false) -> void:
	...
	col.r   += randf_range(-dist *.6, dist)
	col.g   += randf_range(-dist *.6, dist)
	col.b   += randf_range(-dist *.6, dist)
	col.a   *= G.CONFIG.tile_transparency
	col      = G.rgb_smooth(col, smoothness)
	time     = randf_range(time -t, time +t)

	tween_col.tween_property($"%Sprite2D", "modulate", col, time)

	tween_col.tween_property(
		$"%Sprite2D", "material:shader_param/amount", randf_range(2, 3),  time *.5)
	tween_col.tween_property(
		$"%Sprite2D", "material:shader_param/radius", randf_range(-2, 2), time *.5)

	tween_col.chain().tween_property(
		$"%Sprite2D", "material:shader_param/amount", 2.5, time *.5)
	tween_col.tween_property(
		$"%Sprite2D", "material:shader_param/radius", 0.0, time *.5)

	time  *= 2 if not init else 1
	if allow_color_anim:
		tween_col.chain().tween_callback(Callable(self, "tile_color_anim_accents").bind(dist, smoothness, time))
...
func reveal(counter:int) -> void:
	...
	if counter == 0:
		...
		time_mod    = randf_range(-.8, .8)
		delay       = randf_range(.12, .28)
		...
		time_mod    = randf_range(-.6, .6)
		delay       = randf_range(.06, .1)
		reduce_mov  = true
		reduce_rot  = true
	...
	tween_rev.chain().tween_callback(Callable(self, "post_reveal"))

	## blur material applied:
	var mat    := ShaderMaterial.new()
	mat.gdshader  = shader_revealed
...
func change_alpha(time:float, delay:float, trans:int, mod:=1.0) -> void:
	...
	tween_rev.tween_property(
		$"%Sprite2D", "modulate", col, time).set_trans(trans).set_delay(delay)
...
func tile_particle_spawn(type:int=0, lifetime:float=2) -> void:
	...
	PARTICLES_main  = _particles_main.instantiate()
...
#			node1.scale        = Vector2(2,2)
		...
		1:    ## tile helper
			...
			node1.modulate     = G.CONFIG.BG_color *randf_range(.2, .8)
			node1.emitting     = true
			node1.modulate.a   = .6
			node1.speed_scale  = .1
...
func og_tile_shake_anim(count:int, shake_speed:float):
	...
	tween_bump.tween_property(self, "position", new_pos, randf_range(.1, .16) * shake_speed)
	tween_bump.tween_property(self, "position", def_pos, randf_range(.06, .1) * shake_speed)

	if count > 0:
		tween_bump.tween_callback(Callable(self, "og_tile_shake_anim").bind(count-1, shake_speed))
...
func tile_blast_motion(pos:Vector2) -> void:
	var radius        = 260.0
	var power         = randf_range(400, 600)
	var bounce_power  = randf_range(600, 1000)
	...
	if distance < radius:
		...
		rotation_degrees  = randf_range(-40, 40)
